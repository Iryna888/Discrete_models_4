...
int find_way(int matr[N][N], int min[E], int way[N]) {
	int v = 1;
	int u, i = 0;
	stack <int> plan; // план відвідування у вигляді стеку
	plan.push(--v);   // нумеруємо з 0

	matr[v][v] = 1; // відмічаємо, що ця вершина вже заносилась в план 
	int counter = 1;  // початкову вже порахували
	do {
		way[i] = v;
		plan.pop();       // видяляємо її із плану відвідування
		for (u = 0; u < N; u++) { // перебираємо сусідні
			if (matr[v][u] > 0 && !matr[u][u]) { // якщо нова, то
				i++;
				if (u == N) {
					break;
				}
				if (matr[v][N - 1] != 0) {
					u = N - 1;
					plan.push(u);     // добавляєм її в план
					matr[u][u] = 1; // відмічаємо, що вже не нова
					...
					plan.push(u);     // добавляєм її в план
					matr[u][u] = 1; // відмічаємо, що вже не нова
					counter++;        // рахуємо скільки було вершин
					cout << v + 1 << ", " << u + 1 << " - " << matr[v][u] << endl;
					min[u] = matr[v][u];
					v = u;
					way[i] = v;
